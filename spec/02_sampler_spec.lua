...
describe("trace sampler", function()
    ...
    it("applies rate supplied by the agent", function()
        ...
        local rates_applied = sampler:update_sampling_rates(
            '{ "rate_by_service": { "service:test_service,env:": 0.1, "service:,env:": 1.0 } }'
        )
    ...
    it("reports errors when parsing fails", function()
        ...
        local incorrect_key_type =
            sampler:update_sampling_rates('{ "rate_by_service": { { "object-not-key": "should have an error" } }')
        assert.is_false(incorrect_key_type)
        local incorrect_value_type = sampler:update_sampling_rates(
            '{ "rate_by_service": { "service:test_service,env:": 0.1, "service:,env:": "this should be a number"} }'
        )
